...
gulp.task('install', function() {
	...
	if (building) {
		// composed_build = composer({bin: 'composer', cwd: process.cwd()+'/'+paths.build});
		// bowered_build = bower({cwd: paths.build});
	}
	...
	if (building) {
		return merge(composed, bowered, composed_build, bowered_build);
	} else {
